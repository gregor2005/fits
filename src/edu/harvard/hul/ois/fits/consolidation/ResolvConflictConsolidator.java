...
import edu.harvard.hul.ois.fits.consolidation.resolvconflicts.ResolvConflicts;
...
public class ResolvConflictConsolidator implements ToolOutputConsolidator {
	...
	public ResolvConflictConsolidator() throws FitsConfigurationException {
	...
	private List<Element> mergeXmlesults(List<ToolOutput> results, Element element, String mimetype, String fileVersion) {
		...
		//if configured to report conflicts return all values in fitsElements
			...
			fitsElements = ResolvConflicts.prepareElementList(fitsElements, mimetype, fileVersion);
...
//			}
	...
	public FitsOutput processResults(List<ToolOutput> results) {
		...
		for(FitsIdentity identSection : identitySections) {
			...
			mimetype = identSection.getMimetype();
		...
		while((e = findAnElement(culledResults,curSec,false)) != null) {
			List<Element> fitsElements = mergeXmlesults(culledResults, e, mimetype, fileVersion);
